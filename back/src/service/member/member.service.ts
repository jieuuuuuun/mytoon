...
export class MemberService {
  ...
  async join(member: MemberRegisterDTO): Promise<void> {
    ...
    if (member.provider === AuthProvider.LOCAL && member.password) {
      hashedPassword = await this.authService.hashPassword(member.password);
    }
  ...
  async getMember(id: number): Promise<MemberResponse> {
    ...
    const newMember = {
      ...member,
      socials: member.socials.map(({ password, ...rest }) => rest),
    };
    return newMember;
  }

  // 단일 회원 이메일로 조회
  async getMemberByMemberEmail(email: string): Promise<MemberResponse | null> {
    const member = await this.memberRepository.findMemberByMemberEmail(email);
