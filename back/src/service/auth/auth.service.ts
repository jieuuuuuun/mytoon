...
export class AuthService {
  ...
  async socialLogin(socialMember: OAuthLoginDTO) {
    const { email, name, provider, providerId } = socialMember;
    ...
    const foundLocalMember =
      await this.memberService.getMemberByMemberEmail(email);

    // 이미 가입된 회원이라면 -> 토큰 발급
    if (foundSocialMember) {
      const payload: JwtPayload = {
        id: foundSocialMember.id,
        email: foundSocialMember.email,
      };
